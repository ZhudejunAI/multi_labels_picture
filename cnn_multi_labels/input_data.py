...
class DataProcessor(object):
    ...
    def get_dataset(self, image_W, image_H, batch_size, category):
        ...
        # 此时dataset_image 中的一个元素是（image）, 创建了一个DataSet里面都是图片的路径
        dataset_image = tf.contrib.data.Dataset.from_tensor_slices(image_list)

        # 此时dataset_image 中的一个元素是(image_resized), 再对每个路径的图片进行操作 返回新的DataSet
        dataset_image = dataset_image.map(lambda x: self._parse(x, image_W, image_H))

        dataset_label = tf.contrib.data.Dataset.from_tensor_slices(label_list)

        # [(image)(image)...], [(label) (label),..)  == > [(image, label) (image, label)...]
        dataset = tf.contrib.data.Dataset.zip((dataset_image, dataset_label))
